...
@Component({
  selector: 'jhi-mode-paiement',
  templateUrl: './mode-paiement.component.html'
})
export class ModePaiementComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.registerChangeInModePaiements();
  }
  ...
  trackId(index: number, item: IModePaiement): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInModePaiements(): void {
    this.eventSubscriber = this.eventManager.subscribe('modePaiementListModification', () => this.loadPage());
  }

  delete(modePaiement: IModePaiement): void {
    const modalRef = this.modalService.open(ModePaiementDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.modePaiement = modePaiement;
  }
  ...
  protected onSuccess(data: IModePaiement[] | null, headers: HttpHeaders, page: number): void {
    ...
    this.page = page;
    this.router.navigate(['/mode-paiement'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
